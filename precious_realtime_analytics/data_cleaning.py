...
'''
This script reads an excel file, cleans the data, saving it to online_retail.csv using Polars.
Run only is online_retail.csv is not available. 
You can get the input file from https://archive.ics.uci.edu/dataset/352/online+retail
'''
...
# Fill missing 'CustomerID' with '00000'
df = df.with_columns(pl.col('CustomerID').fill_null('00000'))

# Filter rows where InvoiceNo matches the pattern - xxxxxx or Cxxxxxx
...
df.write_csv('online_retail.csv')
